  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        animation: "pop .22s cubic-bezier(.34,1.56,.64,1)", overflow: "hidden",
      }}>
        <div style={{ background: "linear-gradient(135deg,#0F172A,#1E3A5F)", padding: "24px 28px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <h2 style={{ margin: "0 0 10px", fontSize: 18, fontWeight: 800, color: "#fff" }}>{t.bien?.titre || "—"}</h2>
        ...
        <div style={{ padding: "24px 28px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ background: "#F8FAFF", borderRadius: 13, padding: "12px 14px", border: "1px solid #E2E8F0" }}>
              <p style={{ margin: "0 0 4px", fontSize: 10, color: "#94A3B8", fontWeight: 700 }}>🏷️ TYPE</p>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#0F172A" }}>{t.type_transaction === "vente" ? "Vente" : "Location"}</p>
            </div>
          </div>
    ...
    <div style={{ minHeight: "100vh", background: "#F1F5F9", fontFamily: "'Plus Jakarta Sans', sans-serif", paddingLeft: 260 }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "36px 32px", animation: "in .35s ease" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 32, fontWeight: 800, color: "#0F172A" }}>Mes Transactions</h1>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748B" }}>Historique de toutes vos ventes et locations</p>
        </div>

        {/* Stats */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 20 }}>
          {[
            { icon: "🔁", label: "Total",     value: transactions.length,                                         color: "#4F46E5" },
            { icon: "🏠", label: "Ventes",    value: transactions.filter(t => t.type_transaction==="vente").length,    color: "#4F46E5" },
            { icon: "🔑", label: "Locations", value: transactions.filter(t => t.type_transaction==="location").length, color: "#059669" },
          ].map((s) => (
            <div key={s.label} style={{ background: "#fff", borderRadius: 18, padding: "18px 20px", boxShadow: "0 2px 16px rgba(15,23,42,.07)", borderTop: `3px solid ${s.color}` }}>
        ...
        <div style={{ background: "linear-gradient(135deg,#4F46E5,#0891B2)", borderRadius: 18, padding: "20px 28px", marginBottom: 22, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <p style={{ margin: "0 0 4px", fontSize: 12, color: "rgba(255,255,255,.7)", fontWeight: 600 }}>VOLUME TOTAL ENCAISSÉ</p>
            <p style={{ margin: 0, fontSize: 32, fontWeight: 800, color: "#fff" }}>{fmt(volume)}</p>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: "14px 20px", marginBottom: 16, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center", boxShadow: "0 1px 10px rgba(15,23,42,.06)" }}>
          <div style={{ position: "relative", flex: "1 1 240px" }}>
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Rechercher par bien, client…"
        ...
        <div style={{ background: "#fff", borderRadius: 20, overflow: "hidden", boxShadow: "0 2px 16px rgba(15,23,42,.08)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 160px 140px 140px 120px", padding: "12px 24px", background: "#F8FAFF", borderBottom: "1.5px solid #E2E8F0", fontSize: 11, fontWeight: 700, color: "#94A3B8", letterSpacing: ".07em" }}>
          ...
          {!loading && !errLoad && filtered.length === 0 && (
            <div style={{ textAlign: "center", padding: "64px 0", color: "#94A3B8" }}>
              ...
              <p style={{ fontWeight: 600, margin: 0 }}>Aucune transaction</p>
              <p style={{ fontSize: 13, margin: "6px 0 0" }}>Les transactions apparaîtront automatiquement après chaque paiement</p>
            </div>
          )}
          ...
          {!loading && filtered.map((t, i) => {
            ...
            return (
              <div key={t.id_transaction} className="rh" onClick={() => setSelected(t)} style={{
                display: "grid", gridTemplateColumns: "60px 1fr 160px 140px 140px 120px",
                padding: "15px 24px", alignItems: "center",
                borderBottom: i < filtered.length - 1 ? "1px solid #F1F5F9" : "none",
              }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#CBD5E1" }}>#{String(t.id_transaction).padStart(4,"0")}</div>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  ...
                  <div>
                    ...
                    {t.description && <p style={{ margin: 0, fontSize: 11, color: "#94A3B8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 180 }}>{t.description}</p>}
                  </div>
                </div>
                ...
                <div style={{ fontSize: 14, fontWeight: 800, color: "#059669" }}>+{fmt(t.montant)}</div>
